...
export default function Lecture() {
  ...
  const load = useCallback(async (lectureId: string) => {
    try {
      const [info, materials] = await Promise.all([
        invoke<LectureInfo>("get_lecture", { lectureId }),
        invoke<MaterialsBundle>("get_lecture_materials", { lectureId }),
      ]);
      setLecture(info);
      setBundle(materials);
    } catch (e) {
        ...
        prev
          ? {
              ...
              links: prev.links.map((l) =>
                l.id === editingLinkId
                  ? { ...l, title: editingLink.title.trim(), url: editingLink.url.trim() }
                  : l,
              ),
            }
          : prev,
  ...
  if (loading) {
    return <p className="text-sm text-muted-foreground animate-pulse p-4">{t("lecture.loading")}</p>;
  }

  if (loadError || !lecture) {
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">{loadError ?? t("lecture.not_found")}</p>
